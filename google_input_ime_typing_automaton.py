...
def search(inputtable_keys, current, rest_kana, inputted_keys, finished_list):
    if not rest_kana:
        return
    ...
    if current.ime.finished and rest_kana[0] in inputtable_keys:
        ...
        search(inputtable_keys, next_state, next_rest_kana, inputted_keys + [k], finished_list)
        return
            ...
            if output:
                if rest_kana.startswith(output):
                    ...
                    if finished:
                        finished_list.append(next_state)
                    current.connect(k, next_state)
                    search(inputtable_keys, next_state,
                           next_rest_kana, inputted_keys + [k], finished_list)
            else:
                next_state = State(current, new_ime)
                current.connect(k, next_state)
                search(inputtable_keys, next_state, rest_kana, inputted_keys + [k], finished_list)
...
def main():
    ...
    finished_list = []
    search(inputtable_keys, root, target_string, [], finished_list)
